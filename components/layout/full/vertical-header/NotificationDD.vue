  ...
  <v-menu offset-y :close-on-content-click="false">
    ...
    <v-card class="mx-auto" width="380" max-height="500">
      <v-toolbar color="purple">
        <v-toolbar-title>{{ $t('noti.notifications') }}</v-toolbar-title>
      ...
      <v-list>
        <template v-if="myNotifys.length">
          ...
          <v-list-item class="text-center" :to="'/notification'">
            <v-list-item-title>{{ $t('noti.see_more') }}</v-list-item-title>
          </v-list-item>
        </template>
        <template v-else>
          <v-list-item class="text-center">
            <v-list-item-title>{{ $t('noti.no_notifications') }}</v-list-item-title>
          </v-list-item>
        </template>
      </v-list>
    </v-card>
